...
export class HomeScene extends Phaser.Scene {
    ...
    create() {
        ...
        // Calculate tile counts to fit the screen
        const tileSize = 32;
        const tilesX = Math.floor(width / tileSize);
        const tilesY = Math.floor(height / tileSize);

        // Draw background tiles to fit the screen
        ...
        pathTiles.forEach(path => {
            for (let x = path.x; x < path.x + path.width; x++) {
                for (let y = path.y; y < path.y + path.height; y++) {
                    this.add.image(x * tileSize, y * tileSize, 'path').setOrigin(0);
                }
            }
        });

        this.createBuilding(6, 6, 'house1');
        this.createBuilding(15, 6, 'house2');
        this.createBuilding(20, 9, 'house3');
        this.createBuilding(8, 14, 'house1');
        this.createBuilding(25, 15, 'house2');
        this.createBuilding(10, 18, 'house1');
        this.createBuilding(5, 22, 'house3');

        // Use correct asset names for obstacles
        this.createObstacle(2, 10, 'flowers01', 3, 2); // crops1
        this.createObstacle(25, 5, 'flowers02', 4, 3); // crops2
        this.createObstacle(28, 18, 'flowers03', 3, 2); // crops1

        this.createObstacle(4, 4, 'tree03'); // tree1
        this.createObstacle(10, 7, 'tree03'); // tree2
        ...
        // Center camera and disable follow/zoom
    ...
    createWorld() {
        ...
        pathTiles.forEach(path => {
            for (let x = path.x; x < path.x + path.width; x++) {
                for (let y = path.y; y < path.y + path.height; y++) {
                    this.add.image(x * 32, y * 32, 'path').setOrigin(0);
                }
            }
        });
        ...
        this.createObstacle(2, 10, 'flowers01', 3, 2); // crops1
        this.createObstacle(25, 5, 'flowers02', 4, 3); // crops2
        this.createObstacle(28, 18, 'flowers01', 3, 2); // crops1
    ...
    createObstacle(x, y, texture, width = 1, height = 1) {
        this.add.image(x * 32, y * 32, texture).setOrigin(0);
    ...
    createVillager(x, y) {
        const villager = this.add.sprite(x * 32 + 16, y * 32 + 16, 'villager1').setOrigin(0.5);
    ...
    createPlayer() {
        this.player = this.physics.add.sprite(12 * 32 + 16, 8 * 32 + 16, 'player').setOrigin(0.5);
  ...
  // Create player animations
    ...
    removeWalkableTile(x, y) {
        this.walkableTiles = this.walkableTiles.filter(tile => 
            !(tile.x === x && tile.y === y)
        );
    }
    ...
    isWalkable(x, y) {
        ...
        return this.walkableTiles.some(tile => tile.x === tileX && tile.y === tileY);
    }
