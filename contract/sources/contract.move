...
module contract::contract {
    ...
    // Game constants - Updated entrance fee to 0.05 OCT (50000000 = 0.05 * 10^9)
    ...
    /***********************
    * TREASURY & GAMEPLAY (Added entrance fee logic)
    ***********************/
    ...
    /// Represents a single game played by a user.
    ...
    /// A capability that allows a player to claim a reward.
    ...
    /// One-time initializer for treasury
    ...
    /// Start a new game - player pays entrance fee of 0.05 OCT
    public entry fun start_game(
        ...
        // Fixed entrance fee of 0.05 OCT
        assert!(coin::value(&payment) >= ENTRANCE_FEE, E_INSUFFICIENT_FUNDS);

        // Take the entrance fee and add to treasury
        let fee_balance = coin::into_balance(coin::split(&mut payment, ENTRANCE_FEE, ctx));
        balance::join(&mut treasury.balance, fee_balance);
        
        // Return any change to the player
        ...
        event::emit(GameStarted {
            game_session_id: object::id(&game_session),
            player: sender,
            difficulty: difficulty,
            entry_fee: ENTRANCE_FEE,
            timestamp,
        ...
        // Transfer the game session to the player as proof of payment
        transfer::transfer(game_session, sender);
    }

    /// Complete game and set reward amount (called by frontend/admin)
    public entry fun complete_game(
        ...
        // Only the treasury admin can call this function
        assert!(tx_context::sender(ctx) == treasury.admin, E_UNAUTHORIZED);
        assert!(!game_session.is_completed, E_ALREADY_COMPLETED);

        // Validate reward amount doesn't exceed reasonable limits
        let max_reward = ENTRANCE_FEE * 10;
        assert!(reward_amount <= max_reward, E_INVALID_REWARD_AMOUNT);

        game_session.is_completed = true;
        game_session.is_won = won;
        ...
        // Create reward claim if the player won and reward > 0
        if (won && reward_amount > 0) {
            ...
            // Transfer the claim capability to the player
            transfer::transfer(claim, game_session.player);
        }
    }
    ...
    public entry fun claim_reward(
        ...
        let RewardClaim { id, game_session_id: _, player, amount } = claim;
        object::delete(id);

        assert!(tx_context::sender(ctx) == player, E_UNAUTHORIZED);
    ...
    public entry fun fund_treasury(
        ...
        balance::join(&mut treasury.balance, funding_balance);
    }

    /// Admin function to withdraw from treasury
    public entry fun withdraw_from_treasury(
        ...
        transfer::public_transfer(withdrawal_coin, treasury.admin);
    }
    ...
    /// Check if game session is completed
    public fun is_game_completed(session: &GameSession): bool {
        session.is_completed
    }

    /// Check if game was won
    public fun is_game_won(session: &GameSession): bool {
        session.is_won
    }

    /// Get game session player
    public fun get_game_player(session: &GameSession): address {
        session.player
    }
}